...
# from Segnet_weights import get_segnet
from segnet_new import segnet, vgg_segnet
...
class mySegNet(object):
    ...
    def one_hot_lab(self, labels):
        ...
        lab_nd[:, :, 1] = np.floor(vegetation.sum(axis=2) / 3) + np.floor(bare_land.sum(axis=2) / 3)
        lab_nd[:, :, 2] = np.floor(water.sum(axis=2) / 3)
        lab_nd[:, :, 3] = np.floor(road.sum(axis=2) / 3)
    ...
    def create_train_data(self):
        ...
        train_lab = np.ndarray((len(lab_filename), self.img_cols, self.img_rows, self.num_class))
    ...
    def create_test_data(self):
        ...
        test_lab = np.ndarray((len(lab_filename), self.img_cols, self.img_rows, self.num_class))
        for num, name in enumerate(sar_filename):
            img = load_img(name)
            img = img_to_array(img)
            test_data[num] = img
    ...
    def train(self):
        ...
        # model = Deeplabv3(input_shape=(256, 256, 3), classes=5)  # 256, 256, 3 -> 256 256 5
        model = segnet(self.num_class)
        ...
        model_checkpoint = ModelCheckpoint('/emwusr/xianzhengshi/2020_test/research_compare/segnet_weights_4/segnet_val_acc.hdf5', monitor='val_acc', verbose=1,
                                           save_best_only=True, save_weights_only=True)
        model_checkpoint1 = ModelCheckpoint('/emwusr/xianzhengshi/2020_test/research_compare/segnet_weights_4/segnet_train_loss.hdf5', monitor='loss', verbose=1,
                                            save_best_only=True, save_weights_only=True)
        model_checkpoint2 = ModelCheckpoint('/emwusr/xianzhengshi/2020_test/research_compare/segnet_weights_4/segnet_val_loss.hdf5', monitor='val_loss', verbose=1,
                                            save_best_only=True, save_weights_only=True)
